				...
				<h4><?php echo (LANG=='TH')?'ที่อยู่ในการออกใบเสร็จ':"Billing Address";?></h4>
					<table id="address">
						<tbody>
							...
							<tr>
								<td style="text-align: right; padding-right: 20px;"><?php echo (LANG=='TH')?'จังหวัด':"Province";?></td>
								<td><img src="<?php echo base_url()?>public/images/dot.gif" /></td>
								<td>
					...
					<table>
						<tbody>
							...
							<tr>
								...
								<td width="320px"><?php echo (LANG=='TH')?$value->Name_Th:$value->Name_En;?></td>
								<td width="80px"><?php echo ($value->Price!=0)?"{$value->Price} ฿":'FREE';?></td>
							</tr>
							<?php endforeach; ?>
							<tr>
							...
							<tr class="tcl">
								<td><input type="radio" name="how_delivery" <?php echo ($value->How_ID==$order->How_ID)?'checked=checked':''?> value="<?php echo $value->How_ID?>" ></td>
								<td><?php echo (LANG=='TH')?$value->Name_Th:$value->Name_En;?><br /><?php echo (LANG=='TH')?$value->Description_Th:$value->Description_En;?></td>
								<td><?php echo (number_format(cal_range_weight($value->How_ID, $order->Total_Weight))==0)?'FREE':number_format(cal_range_weight($value->How_ID, $order->Total_Weight)).' ฿';?></td>
							</tr>
							<?php endforeach; ?>
						</tbody>
					</table>
			...
			<div class="right">
								...
								<div id="char_right">
									<table width="200px">
										<tbody>
											...
											<tr>
												<td style="	text-align: right;"><span class="price"><?php echo number_format($result->order_item_Total_Price)?> ฿</span></td>
											</tr>
										</tbody>
									</table>
								</div>
				...
				<table width="400px" id="price_total">
					<tbody>
						...
						<tr>
							<td>Subtotal</td>
							<td style="text-align: right; padding-right: 50px;"><?php echo number_format($order->Total_Price)?> ฿</td>
							<td style="text-align: center;"><a href="<?php echo $this->session->userdata('prev_url')?>">Back to Shopping</a></td>
						</tr>
						<tr>
							<td>Shipping</td>
							<td style="text-align: right; padding-right: 50px;"><?php echo number_format(cal_range_weight($order->How_ID, $order->Total_Weight))?> ฿</td>
							<td></td>
						</tr>
						<tr>
							<td>Services</td>
							<td style="text-align: right; padding-right: 50px;"><?php echo number_format(cal_price_option($order->Order_ID))?> ฿</td>
							<td></td>
						</tr>
						<tr>
							<td style="font-weight: bold;"><h4>Total</h4></td>
							<td style="text-align: right; padding-right: 50px; font-weight: bold;"><h4><?php echo number_format($order->Total_Price + cal_range_weight($order->How_ID, $order->Total_Weight) + cal_price_option($order->Order_ID))?> ฿</h4></td>
