...
<?php endif;?>
		   	<?php echo form_open('my/info_update')?>
		   	<input type="hidden" name="Cus_ID" value="<?php echo set_value('Cus_ID', '')?>"/>
		   	<input type="hidden" name="Shipping_ID" value="<?php echo set_value('Shipping_ID', '')?>"/>
			<!-- Billing Address -->
					...
					<div class="viewport">
						...
						?>
								...
								<table>
									<tbody>
									<tr>
										<td width="78%"><h4><?php echo (LANG=='TH')?'ที่อยู่ในการจัดส่งสินค้า':"Shipping Address";?></h4></td>
										<td><div id="checkbox">
								<input type="checkbox" name="c1" id="c1" onclick="javascript:same_data()" value="1" > &nbsp;<?php echo (LANG=='TH')?'ใช้ที่อยู่เดียวกับการออกใบเสร็จ':"Same as Billing Address";?>
								</div></td>
									</tr>
									</tbody>
								</table>
								...
								<table>
									<tbody>
											...
											<td>
												<div class="styled-select">
													<select name="s_Country_ID" id="s_Country_ID" onchange="change_country('s_Country_ID', 's_City_ID_cb', 's_City_Name_txt', 's_city_prov_th', 's_city_prov_en');">
														...
														foreach($queryCountry as $valueCountry)
														{ ?>
															<option value="<?php echo $valueCountry->Country_ID?>" <?php echo set_select('s_Country_ID',$valueCountry->Country_ID)?>>
																<?php echo (LANG=='TH')?"{$valueCountry->country_name_th}":"{$valueCountry->country_name}";?>
															</option>
														<?php } ?>
								                    </select>
										...
										<tr>
											...
											<td><img src="<?php echo base_url()?>public/images/dot.gif" /></td>
											<td>
												<div class="styled-select" id="s_City_ID_cb" style="display:<?php echo (set_value('s_Country_ID', '')=='222')?'block':'none';?>;">
												...
												<div id="s_City_Name_txt" style="display:<?php echo (set_value('s_Country_ID', '')=='222')?'none':'block';?>;">
